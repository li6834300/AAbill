...
describe('注册需验证邮箱', () => {
  ...
  it('验证后即可正常登录', async () => {
    ...
    expect((await app.request(post('/auth/login', creds('a@example.com')))).status).toBe(200);
  });

  it('伪造令牌 400', async () => {
    const { app } = makeApp();
    await app.request(post('/auth/register', creds('a@example.com')));
    const res = await app.request(post('/auth/verify', { token: 'f'.repeat(64) }));
  ...
  it('令牌只能用一次', async () => {
    ...
    expect((await app.request(post('/auth/verify', { token: tk }))).status).toBe(200);
    expect((await app.request(post('/auth/verify', { token: tk }))).status).toBe(400);
  });
  ...
  it('可重发验证信,旧令牌随即失效', async () => {
    ...
    await app.request(post('/auth/resend-verification', { email: 'a@example.com' }));
    const fresh = tokenFromMail(mailer);
    expect(fresh).not.toBe(old);
    expect((await app.request(post('/auth/verify', { token: old }))).status).toBe(400);
    expect((await app.request(post('/auth/verify', { token: fresh }))).status).toBe(200);
  });
...
describe('每天 10 个已验证账号上限', () => {
  ...
  it('名额满时注册就被挡下(不必等收信点链接)', async () => {
    ...
    const res = await app.request(post('/auth/register', creds('late@example.com')));
    expect(res.status).toBe(429);
  ...
  it('未验证的注册不消耗名额', async () => {
    const { app, mailer } = makeApp({ now: () => day(5) });
    for (let i = 0; i < 20; i++) {
      await app.request(post('/auth/register', creds(`pending${i}@example.com`)));
    }
  ...
  it('因名额被拒的令牌次日仍可验证', async () => {
    ...
    // 名额被别人占满
    for (let i = 0; i < 10; i++) await signUp(app, mailer, `u${i}@example.com`);
    expect((await app.request(post('/auth/verify', { token: tk }))).status).toBe(429);
