...
describe('FamilyChips', () => {
  it('渲染全部家庭名', () => {
    render(<FamilyChips families={families} onAdd={jest.fn()} onRemove={jest.fn()} />);
  ...
  it('输入名字点添加 → onAdd(name) 并清空输入', () => {
    const onAdd = jest.fn();
    render(<FamilyChips families={families} onAdd={onAdd} onRemove={jest.fn()} />);
  ...
  it('点家庭的删除 → onRemove(id)', () => {
    ...
    render(<FamilyChips families={families} onAdd={jest.fn()} onRemove={onRemove} />);
